...
public class PiecePanel extends JLayeredPane {
    ...
    public PiecePanel(){
        ...
        this.generateBoard("8/8/8/3b4/4B3/8/8/8 w KQkq - 0 1");
    }
    ...
    private void generateBoard(String FEN){
        ...
        for (char c : piecePositions) {
            ...
            switch (Character.toLowerCase(c)) {
                case 'r' -> currentPiece = new Rook(team, Board.fields[currentIndex]);
                case 'n' -> currentPiece = new Knight(team, Board.fields[currentIndex]);
                case 'b' -> currentPiece = new Bishop(team, Board.fields[currentIndex]);
                case 'q' -> currentPiece = new Queen(team, Board.fields[currentIndex]);
                case 'k' -> currentPiece = new King(team, Board.fields[currentIndex]);
                case 'p' -> currentPiece = new Pawn(team, Board.fields[currentIndex]);
            }
